...
static pcl::PointCloud<pcl::PointXYZ>::Ptr loadCloud(const std::string& path) {
    ...
    if (cloud->points.size() < 3) throw std::runtime_error("Too few points in cloud");
    return cloud;
}
...
static void splitGroundWalls(pcl::PointCloud<pcl::PointXYZ>::Ptr src,
    ...
    if (inliers.indices.empty()) throw std::runtime_error("RANSAC plane segmentation failed");
...
static std::vector<Eigen::Vector2d> to2DPoints(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud) {
    ...
    for (const auto& p : cloud->points) pts.emplace_back(p.x, p.y);
    return pts;
}
...
static cv::Mat rasterizePointsToCounts32F_AA(const std::vector<Eigen::Vector2d>& pts2d,
    ...
    for (const auto& p : pts2d) {
        ...
        for (int xi = 0; xi < 2; ++xi) {
            const int x = xs[xi];
            if (x < 0 || x >= hiRows) continue;
            for (int yi = 0; yi < 2; ++yi) {
                const int y = ys[yi];
                if (y < 0 || y >= hiCols) continue;
                const float w = wx[xi] * wy[yi];
                hi.at<float>(x, y) += w;
            }
        }
...
static cv::Mat morphologicalCloseImage(const cv::Mat& input_img, int kernel_size, int iterations) {
    ...
    if (iterations == 0) return input_img;
...
static cv::Mat filterConnectedComponents(const cv::Mat& input_img,
    ...
    if (adaptive) {
        ...
        if (min_area < 10.0) min_area = 10.0;
...
static cv::Mat makeLineBinaryFromProjection(const cv::Mat& grayU8,
    ...
    if (medianKSize > 1) {
        if (medianKSize % 2 == 0) ++medianKSize;
...
static cv::Mat redrawMergedHoughLines(const cv::Mat& thin,
    ...
    if (rawSegs.empty()) return merged;
    ...
    for (const auto& seg : rawSegs) {
        ...
        for (int i = 0; i < static_cast<int>(clusters.size()); ++i) {
            ...
            if (!fitClusterLine(clusters[i], linePoint, lineDir, tMin, tMax, lineAngle)) continue;

            if (angleDifferenceDeg(angle, lineAngle) > mergeAngleDeg) continue;

            const cv::Point2f d = mid - linePoint;
            const double perpDist = std::fabs(d.x * lineDir.y - d.y * lineDir.x);
            if (perpDist > mergeDistPx) continue;
    ...
    for (const auto& cluster : clusters) {
        cv::Point2f linePoint, lineDir;
        double tMin = 0.0, tMax = 0.0, lineAngle = 0.0;
        if (!fitClusterLine(cluster, linePoint, lineDir, tMin, tMax, lineAngle)) continue;
...
static pcl::PointCloud<pcl::PointXYZ>::Ptr maskToPointCloud2D(const cv::Mat& mask,
    ...
    if (mask.empty() || gridSize <= 0.0) return out;
...
static int runPcdProjection(const ProjectionOptions& opt) {
    ...
    if (opt.inputPath.empty()) throw std::runtime_error("inputPath is empty");
    if (opt.outDir.empty()) throw std::runtime_error("outDir is empty");
    ...
    splitGroundWalls(cloud, ground, walls, opt.groundDistThresh, opt.groundRansacN, opt.groundMaxIter);
    std::cout << "[projection] ground: " << ground->points.size() << " | walls: " << walls->points.size() << "\n";

    PCD2d pcd2d(walls, ground);
    pcd2d.SetHeightBand(opt.lowerZ, opt.upperZ);
    pcd2d.SetVoxelSize(opt.voxel);

    std::cout << "[projection] height band: [" << opt.lowerZ << ", " << opt.upperZ << "], voxel: " << opt.voxel << "\n";
    ...
    auto walls_raw = pcd2d.getWallsRaw();
    auto walls_cropped = pcd2d.getWallsCropped();
    ...
    if (walls_raw && !walls_raw->points.empty()) {
        pcl::io::savePCDFileBinary((fs::path(opt.outDir) / "walls_raw_rp.pcd").string(), *walls_raw);
    }
    if (walls_cropped && !walls_cropped->points.empty()) {
        pcl::io::savePCDFileBinary((fs::path(opt.outDir) / "walls_cropped_rp.pcd").string(), *walls_cropped);
    }
    ...
    PointRaster raster(pts2d, effectiveGridSize);

    // Anti-aliased rasterization (bilinear splat) to reduce stair-step artifacts,
    // while keeping the same raster bounds, image size, and downstream logic.
    const int ss = opt.sharpRaster ? 1 : chooseSupersampleFactor(raster.getSize());
    const cv::Mat imgF_counts = rasterizePointsToCounts32F_AA(pts2d,
                                                              raster.getOrigin(),
                                                              raster.getSize(),
                                                              raster.getGridSize(),
                                                              ss);
    ...
    cv::minMaxLoc(imgF_counts, &minVal, &maxVal);
    if (opt.sharpRaster) {
        cv::threshold(imgF_counts, img, 0.0, 255.0, cv::THRESH_BINARY);
        img.convertTo(img, CV_8U);
    } else {
        cv::Mat imgF = imgF_counts;
        if (maxVal > 0.0) {
    ...
    if (opt.saveLineBinary) {
        ...
        if (opt.exportLineLikePcd) {
            auto walls2dLine = maskToPointCloud2D(lineForExport, raster.getOrigin(), raster.getGridSize());
    ...
    if (walls_raw) {
        std::cout << "[projection] saved walls_raw_rp.pcd (" << walls_raw->points.size() << ")\n";
    }
    if (walls_cropped) {
        std::cout << "[projection] saved walls_cropped_rp.pcd (" << walls_cropped->points.size() << ")\n";
    }
...
int main(int argc, char** argv) {
    ...
    if (argc >= 10) opt.occThreshold = std::stoi(argv[9]);
    if (argc >= 11) opt.medianKSize = std::stoi(argv[10]);
    if (argc >= 12) opt.binDilate = std::stoi(argv[11]);
    if (argc >= 13) opt.binClose = std::stoi(argv[12]);
    ...
    if (argc >= 14) opt.closeKernelSize = std::stoi(argv[13]);
    if (argc >= 15) opt.ccEnable = (std::stoi(argv[14]) != 0);
    if (argc >= 16) opt.ccMinArea = std::stod(argv[15]);
    if (argc >= 17) opt.ccMaxArea = std::stod(argv[16]);
    if (argc >= 18) opt.ccAspectRatio = std::stod(argv[17]);
    if (argc >= 19) opt.ccAdaptive = (std::stoi(argv[18]) != 0);
    if (argc >= 20) opt.exportLineLikePcd = (std::stoi(argv[19]) != 0);

    if (argc >= 21) opt.useXimgprocThinning = (std::stoi(argv[20]) != 0);
    if (argc >= 22) opt.houghThreshold = std::stoi(argv[21]);
    if (argc >= 23) opt.houghMinLineLength = std::stod(argv[22]);
    if (argc >= 24) opt.houghMaxLineGap = std::stod(argv[23]);
    if (argc >= 25) opt.mergeAngleDeg = std::stod(argv[24]);
    if (argc >= 26) opt.mergeDistPx = std::stod(argv[25]);
    if (argc >= 27) opt.mergeGapPx = std::stod(argv[26]);
    if (argc >= 28) opt.sharpRaster = (std::stoi(argv[27]) != 0);
