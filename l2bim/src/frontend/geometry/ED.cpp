...
/**
 * @brief 从图像创建ED对象并执行边缘检测
 * @param _srcImage 输入图像，应为灰度图
 * @param _op 梯度算子，可选值：PREWITT_OPERATOR, SOBEL_OPERATOR, SCHARR_OPERATOR, LSD_OPERATOR，默认为PREWITT_OPERATOR
 * @param _gradThresh 梯度阈值，用于边缘检测，默认为20
 * @param _anchorThresh 锚点阈值，用于确定锚点，默认为0
 * @param _scanInterval 扫描间隔，默认为1
 * @param _minPathLen 最小路径长度，默认为10
 * @param _sigma Gaussian平滑的sigma值，默认为1.0
 * @param _sumFlag 求和标志，默认为true
 * @details 从输入图像创建ED对象，执行完整的边缘检测流程，包括梯度计算、锚点检测和锚点连接
 */
...
{	// 检查参数有效性，确保参数在合理范围内
	if (_gradThresh < 1) _gradThresh = 1; // 梯度阈值不能小于1
	if (_anchorThresh < 0) _anchorThresh = 0; // 锚点阈值不能小于0
	if (_sigma < 1.0) _sigma = 1.0; // sigma值不能小于1.0
	...
	// 初始化边缘检测参数
	op = _op;                     // 梯度算子类型
	gradThresh = _gradThresh;      // 梯度阈值
	anchorThresh = _anchorThresh;  // 锚点阈值
	scanInterval = _scanInterval;  // 扫描间隔
	minPathLen = _minPathLen;      // 最小路径长度
	sigma = _sigma;                // Gaussian平滑的sigma值
	sumFlag = _sumFlag;            // 求和标志

	// 初始化线段计数器和存储结构
	segmentNos = 0; // 线段数量初始化为0
	segmentPoints.push_back(vector<Point>()); // 创建空的线段存储向量

	// 初始化边缘检测所需的图像
	edgeImage = Mat(height, width, CV_8UC1, Scalar(0)); // 边缘图像，初始化为全黑
	smoothImage = Mat(height, width, CV_8UC1);           // 平滑后的图像
	gradImage = Mat(height, width, CV_16SC1);           // 梯度图像，存储short类型的梯度值

	// 获取图像数据指针
	srcImg = srcImage.data; // 源图像数据指针

	//// 使用Edge Drawing算法检测边缘  ////
	
	/*------------ 使用Gaussian核平滑图像 -------------------*/
	if (sigma == 1.0)
		GaussianBlur(srcImage, smoothImage, Size(5, 5), sigma); // sigma=1.0时使用固定5x5内核
	else
		GaussianBlur(srcImage, smoothImage, Size(), sigma);     // 其他sigma值时自动计算内核大小

	// 从Mat对象获取数据指针
	smoothImg = smoothImage.data; // 平滑图像数据指针
	gradImg = (short*)gradImage.data; // 梯度图像数据指针，转换为short类型
	edgeImg = edgeImage.data; // 边缘图像数据指针

	// 分配方向图像内存
	dirImg = new unsigned char[width*height]; // 方向图像，存储每个像素的边缘方向

	/*------------ 计算梯度和边缘方向图 -------------------*/
	ComputeGradient(); // 调用ComputeGradient函数计算梯度图像和方向图像

	/*------------ 计算锚点 -------------------*/
	ComputeAnchorPoints(); // 调用ComputeAnchorPoints函数检测边缘锚点

	/*------------ 连接锚点形成边缘段 -------------------*/
	JoinAnchorPointsUsingSortedAnchors(); // 调用JoinAnchorPointsUsingSortedAnchors函数连接锚点
...
/**
 * @brief ED类的复制构造函数
 * @param cpyObj 要复制的ED对象
 * @details 用于EDLines和EDCircle类，当ED对象作为构造函数参数时使用，只复制必要的属性
 */
 ...
 * @param _gradImg 梯度图像指针，包含short类型的梯度值
 * @param _dirImg 方向图像指针，包含uchar类型的方向值
 * @param _width 图像宽度
 * @param _height 图像高度
 ...
 * @details 用于EDColor类，从给定的梯度图像和方向图像中查找边缘图像
 */
 ...
 * @param obj EDColor对象引用
 * @details 从EDColor对象创建ED对象，复制其宽度、高度和线段信息
 */
ED::ED(EDColor &obj) 
{
	width = obj.getWidth();
