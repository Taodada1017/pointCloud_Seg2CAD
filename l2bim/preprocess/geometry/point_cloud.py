...
class PCD2d:
    ...
    def reset_rp(self, lidar_origin="default") -> np.ndarray:
        ...
        if len(down_ground.points) < 3:
            # 高度裁切后的点云常常没有足够地面点，无法拟合地面平面。
            # 这时如果直接用单位阵，cfg 的 min_height/max_height 往往会按“全局Z”裁掉全部点。
            # 因此回退为：只做 Z 平移，让 walls 的最小 Z 归零，使高度阈值在局部坐标系下生效。
            walls_np = np.asarray(self.walls_pcd.points)
            if walls_np.shape[0] < 3:
                print("No enough points to fit the plane and walls too small.")
                return False
            z_min = float(walls_np[:, 2].min())
            T = np.eye(4)
            T[2, 3] = -z_min
            print(f"No enough points to fit the plane. Fallback to Z-shift (z_min={z_min:.6f}).")
            self.T_rp = T
    ...
    def crop_walls(self):
        ...
        if len(self.og_walls_cropped.points) == 0 and len(self.og_walls.points) > 0:
            pts = np.asarray(self.og_walls.points)
            z_min = float(pts[:, 2].min())
            z_max = float(pts[:, 2].max())
            print(
                "[PCD2d] 警告: 高度裁剪后点为空，回退为不裁剪。"
                f" (cfg.min_height={self.lower_bound}, cfg.max_height={self.upper_bound}, z_range=[{z_min:.3f},{z_max:.3f}])"
            )
            self.og_walls_cropped = self.og_walls
