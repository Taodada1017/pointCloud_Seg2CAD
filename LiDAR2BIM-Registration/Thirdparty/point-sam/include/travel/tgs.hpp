...
namespace travel {
    ...
    public:
        ...
        void setParams(const double max_range, const double min_range) {
            ...
            TH_NORMAL_ = 0.940;
            TH_WEIGHT_ = 200;
            TH_LCC_NORMAL_SIMILARITY_ = 0.03;
            TH_LCC_PLANAR_MODEL_DIST_ = 0.1;
            TH_OBSTACLE_HEIGHT_ = 1.0;
            REFINE_MODE_ = true;
            VIZ_MDOE_ = true;
        ...
        void estimateGround(const pcl::PointCloud<PointType>& cloud_in,
			...
			// visualization of cloudGround_out
//			 pcl::visualization::PCLVisualizer viewer("PCL Viewer");
//			 viewer.setBackgroundColor(0.0, 0.0, 0.0);
//			 viewer.addPointCloud<pcl::PointXYZ>(cloudGround_out.makeShared(), "cloudGround_out");
//			 viewer.spin();
    ...
    private:
		...
		// after revertTraversableNodes, the tgf_out is updated with the refined planar model, but there are some nodes that are too high, we only keep the low nodes as the traversable nodes
		void removeHighNodes(TriGridField<PointType>& tgf_out){
			// first we don't know the lowest height of the GROUND nodes, we need to find the lowest height
			double min_height = 10;
			for (int r_i = 0; r_i < rows_; r_i++) {
				for (int c_i = 0; c_i < cols_; c_i++) {
					for (int s_i = 0; s_i < (int) tgf_out[r_i][c_i].size(); s_i++) {
						if (tgf_out[r_i][c_i][s_i].node_type != GROUND) { continue; }
						if (tgf_out[r_i][c_i][s_i].mean_pt[2] < min_height && tgf_out[r_i][c_i][s_i].mean_pt[2] > config::min_height){
							min_height = tgf_out[r_i][c_i][s_i].mean_pt[2]; // we don't consider the nodes that are too low ,elevator, etc.
						}
					}
				}
			}
//			std::cout << "min_height: " << min_height << std::endl;
			// then we remove the high nodes
			for (int r_i = 0; r_i < rows_; r_i++) {
			for (int c_i = 0; c_i < cols_; c_i++) {
			for (int s_i = 0; s_i < (int) tgf_out[r_i][c_i].size(); s_i++) {
				if (tgf_out[r_i][c_i][s_i].node_type != GROUND) { continue; }
				if (tgf_out[r_i][c_i][s_i].mean_pt[2] > min_height + 0.5){
					tgf_out[r_i][c_i][s_i].node_type = NONGROUND;
				}
			}
			}
